...
namespace DAL
{
    ...
    public class DomainDAL
    {
        ...
        ///表名
        /// <summary>
        private static string TableName = "Domain";
        ...
        public static List<T> GetList2<T>(string SelectIF, string strWhere, string OrderName = "")
        {
            string sqlText = " SELECT " + SelectIF + " FROM " + TableName + " WHERE " + strWhere;
            if (!string.IsNullOrEmpty(OrderName))
            {
                sqlText += " ORDER BY  " + OrderName;
            }

            DataTable dt = DBCommon.DBHelper.GetDataSet(DALUtil.ConnString, sqlText).Tables[0];
            return DALUtil.ConvertDataTableToEntityList<T>(dt);
        }
        #endregion
        ...
        /// <param name="ID">ID</param>
        /// <param name="SelectIF">查询字段</param>
        public static DomainEntity Get_99(int ID, string SelectIF)
        {
            try
            {
                //参数Where条件
                ...
                return Get1<DomainEntity>(SelectIF, pramsWhere);
            }
            catch { return null; }
        }
        #endregion
